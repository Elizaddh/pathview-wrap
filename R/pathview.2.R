...
pathview.2 <- function( run ,diff.tool, gene.data= cnts, ref, samp, gsets, pathway.id,  both.dirs = list(gene = T, cpd = T), plot.gene.data, outname, compare="paired")
{
  ...
  if(is.null(pathway.id)==FALSE){
    ...
    if (run =="complete")
    {
      ...
      fc.kegg.p <- run_path_analysis(logfoldchange, gsets,ref=NULL, samp= NULL, compare )#, gene.data = gene.data, ref, samp, plot.gene.data = T )
      ...
      fc.kegg.p <- run_path_analysis(gene.data, gsets,ref = ref, samp = samp, compare)#, gene.data = gene.data, ref, samp, plot.gene.data = T  )
      print("now pathview")
      path.ids.2<- rownames(fc.kegg.p$greater)[fc.kegg.p$greater[, "q.val"] < 0.1 &
                                                 + !is.na(fc.kegg.p$greater[, "q.val"])]
